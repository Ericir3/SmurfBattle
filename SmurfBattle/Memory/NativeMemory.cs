...
namespace Memory
{
	...
	public abstract class NativeMemory : IDisposable
	{
		...
		protected NativeMemory(IntPtr process, bool createInjector = false)
		{
			Requires.NotNull(process.ToString(), nameof(process));
            /*
			Process = process;
        ...
        public void WriteString(IntPtr ıntPtr)
        {
...
//			Injector?.Dispose();
	...
	//		Process.LeaveDebugMode();
        ...
        public virtual string ReadQQQ(IntPtr address)
        {
            Requires.NotEqual(address, IntPtr.Zero, nameof(address));
            //Requires.NotNull(encoding, nameof(encoding));
            string aq = ReadQQ(address);
            return aq;
        }
        /// <summary>
        ...
        public abstract string ReadQQ(IntPtr address);
        /// <summary>
