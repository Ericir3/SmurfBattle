...
namespace Memory
{
	...
	public sealed class ExternalProcessMemory : NativeMemory
	{
		...
		/// </exception>
		public ExternalProcessMemory(IntPtr process,
			ProcessAccess access, bool createInjector = false) : base(process, false)
		{
            //IntPtr dmm = new IntPtr(21);
            ProcessHandle = process;// OpenProcess(access, false, process.Id);
            new SafeMemoryHandle(process);
		...
		///     not been set.-or- There is no process associated with this <see cref="T:System.Diagnostics.Process" /> object.
		/// </exception>
		public ExternalProcessMemory(IntPtr process, bool createInjector = false) : this(process,
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern unsafe bool ReadProcessMemory(
            IntPtr hProcess, IntPtr lpBaseAddress, [MarshalAs(UnmanagedType.LPWStr)]out string lpBuffer, int dwSize, out int lpNumberOfBytesRead);
		...
		private static extern bool VirtualProtectEx(
            IntPtr hProcess, IntPtr lpAddress, IntPtr dwSize, uint flNewProtect, out uint lpflOldProtect);
        ...
        public override unsafe string ReadQQ(IntPtr address)
        {
            // We should consider not throwing if the amount of bytes read doesn't match the expected amount of bytes.
            // It is perfectly possible for user code to handle these scenarios - we just have to think of a proper way of notifying them.
            ...
            if (ReadProcessMemory(ProcessHandle, address, out buffer, 64, out numRead))
                    return buffer;
            return null;
            //throw new BlueRainReadException(address, count);
        }
        ...
        /// <param name="isRelative">if set to <c>true</c> [is relative].</param>
        /// <returns></returns>
        /// <exception cref="BlueRainWriteException"></exception>
        ...
        public override void WriteBytes(IntPtr address, byte[] bytes, bool isRelative = false)
		{
            ...
            if (!success)
                Console.WriteLine("Yazma Başarısız");    //throw new BlueRainWriteException(address, bytes.Length);
		}
